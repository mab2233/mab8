...
async def p_cb(b, cb):
    ...
    if type_ == "playlist":
        ...
        if not queue:
            await cb.message.edit("**  مفيش حاجه شغاله يهبل 😂 ❗**")
        temp = []
        for t in queue:
            temp.append(t)
        now_playing = temp[0][0]
...
async def m_cb(b, cb):
    ...
    if type_ == "pause":
        ...
        if (
            ...
            await cb.answer("وقفنا الاغاني نعم !")
...
async def play(_, message: Message):
    ...
    if message.chat.id in DISABLED_GROUPS:
        return    
    lel = await message.reply("🔄 اصبر يعم بدور ع الاغنيه ...**")
        ...
        keyboard = InlineKeyboardMarkup(
            [
                [
                    ...
                    InlineKeyboardButton("📣 ᴄʜᴀɴɴᴇʟ", url=f"https://t.me/xO_mab_Ox")
                ],
            ]
        )
        requested_by = message.from_user.first_name
